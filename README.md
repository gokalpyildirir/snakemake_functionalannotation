...
I recommend using Mamba or Micromamba to create a conda environment that houses (almost) all the necessary tools for this analysis. Follow these commands to set up the environment and activate it for your analysis:

```bash
micromamba create -n AAFC_assembly_annotation_env --file environment.yaml
micromamba activate AAFC_assembly_annotation_env
```
## Placing the Illumina Reads into the Illumina_reads Folder

This workflow is crafted to handle multiple Illumina paired-end libraries. To utilize it effectively, consolidate all libraries associated with a particular sample into the designated "Illumina_reads" folder. The workflow seamlessly processes all libraries and trims, filters, and assembles these reads into genome assemblies.

## Requirements and Dependencies for the Workflow

### Databases

TTo effectively use this workflow, ensure that you have the Kraken2 standard database and the BUSCO fungi odb10 database. Instructions for downloading the Kraken2 databases are available [here](https://github.com/DerrickWood/kraken2) a while instructions for acquiring BUSCO databases can be found [here](https://gitlab.com/ezlab/busco). 

After downloading these databases, take note of their respective locations. Modify the ```config.yaml``` file accordingly (see Modifying the Configuration File section below) by specifying the paths to these databases before initiating the pipeline. This ensures seamless integration and proper functioning of the workflow.

### Dependencies

Functional annotations require the use of InterProScan5. However, incorporating InterProScan5 directly into ```funannotate annotate``` isn't feasible due to its Linux-only compatibility. Detailed information on this limitation is available [here](https://funannotate.readthedocs.io/en/latest/annotate.html). 

The recommended approach for utilizing InterProScan5 is running this tool through its [Docker image](https://hub.docker.com/r/interpro/interproscan), then running ```funannotate annotate``` for functional annotations. To accomplish this, after utilizing the tool within Docker, note the location of the output XML file generated by InterProScan5.

Lastly, update the ```config.yaml``` file by specifying the path to this XML file. This step is crucial for subsequent stages of the pipeline to utilize the InterProScan5 results effectively.
...
busco_odb_path: /path/to/fungi_odb10
```
...
```yaml
trinity_ev: /path/to/trinity_out_dir.Trinity.fasta
protein_ev: /path/to/sequence.fasta
```
...
```shell
# You need to change the Kraken2 db path here (modify /path/to/kraken2_standard_db)
...
do
    ...
    kraken2 -db /path/to/kraken2_standard_db --threads 16 --unclassified-out $readkraken \
...
1. Place your paired-end Illumina reads into the Illumina_reads folder. Make sure that all libraries belong to the same sample.
...
3. Visualize the Directed Acyclic Graph (DAG) for the pipeline:
...
4. If dry run completes without a problem and everything looks as expected in the DAG, you can proceed to run the pipeline:
...
Once orthologous groups are identified, [the next part of this pipeline](https://github.com/gokalpyildirir/Snakemake_wf) can be run to construct the phylogenetic tree.
